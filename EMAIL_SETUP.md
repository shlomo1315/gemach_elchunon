...
## חלופה: סיסמת אפליקציה (אם מפעילים אימות דו-שלבי)

אם בעתיד תעדיף דרך פשוטה יותר: מפעילים אימות דו-שלבי בחשבון, יוצרים
"סיסמת אפליקציה" ב-https://myaccount.google.com/apppasswords, ומגדירים רק:
`GMAIL_USER` + `GMAIL_APP_PASSWORD`. הקוד תומך בשתי הדרכים אוטומטית
(אם מוגדרים פרטי OAuth2 — הם קודמים).
